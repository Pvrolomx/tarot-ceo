...
const CardPlaceholder = ({ card, isReversed, isFlipped, onClick, lang }: { card: typeof MAJOR_ARCANA[0] | null; isReversed: boolean; isFlipped: boolean; onClick: () => void; lang: 'en' | 'es' }) => {
  const t = T[lang];
  return (
        ...
        <div className={`card-back w-full h-full rounded-xl bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-amber-500/50 flex flex-col items-center justify-center p-3 ${isReversed ? 'rotate-180' : ''}`}>
          {card && (
            <>
              ...
              <div className="text-xs font-semibold text-amber-400 text-center">{lang === 'es' ? card.nameEs : card.name}</div>
              {isReversed && <div className="text-[10px] text-red-400 mt-1">({t.reversed})</div>}
            </>
          )}
        </div>
      </div>
            ...
            <div className="flex justify-center gap-4 md:gap-8 flex-wrap mb-8">
              {drawnCards.map((drawn, i) => (
                <div key={i} className="text-center">
                  <CardPlaceholder card={flippedCards[i] ? drawn.card : null} isReversed={drawn.reversed} isFlipped={flippedCards[i]} onClick={() => handleFlipCard(i)} lang={lang} />
                  ...
                  {flippedCards[i] && (
                    <div className="mt-1 text-xs text-slate-400 max-w-[160px]">
                      {(lang === 'es' ? drawn.card.keywordsEs : drawn.card.keywords).join(' • ')}
                    </div>
                  )}
                </div>
              ))}
